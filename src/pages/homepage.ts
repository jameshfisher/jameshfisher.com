...
export function renderHomepage(posts: Post[]) {
  ...
  const hnFavorites = posts
    .filter(
      (post) =>
        !!(post.frontmatter.hnUrl && post.frontmatter.hnUpvotes && post.frontmatter.hnUpvotes > 100)
    )
    .sort((a, b) => (b.frontmatter.hnUpvotes ?? 0) - (a.frontmatter.hnUpvotes ?? 0));

  const myFavoritePosts = allPosts.filter(
    (post) =>
      (post.frontmatter.tags ?? []).includes("fave") && !hnFavorites.includes(post)
  );
  ...
  return h("html", { lang: "en" }, [
    ...
    h(
      "body",
      { class: "experiment-dont-show-link-summaries", style: "margin: 1em" },
      [
        navbar,
        h(
          "video",
