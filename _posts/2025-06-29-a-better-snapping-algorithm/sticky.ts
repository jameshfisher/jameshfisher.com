...
const updateSnapState = (draggedLeft: number, draggedTop: number, currentSnapState: DraggingState, previousLeft: number, previousTop: number): DraggingState => {
...
const getSnapLineForAxis = (position: number, axis: AxisSnapState, isVertical: boolean = true): number | null => {
...
const getDistanceToSnapLine = (position: number, axis: AxisSnapState, snapLine: number, isVertical: boolean = true): number => {
...
const calculateSnappedPosition = (draggedLeft: number, draggedTop: number, snapState: DraggingState) => {
  const snapLines = getSnapLines();
...
const drawWindow = (ctx: CanvasRenderingContext2D, left: number, top: number, windowType: 'original' | 'dragged' | 'snapped') => {
...
const draw = () => {
  const canvas = document.getElementById('sticky-snap-app') as HTMLCanvasElement;
